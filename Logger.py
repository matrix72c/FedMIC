...
def init_logger():
    ...
    client_loss_df.to_csv("./result/obfus_client_loss.csv", mode='w', index=False)

    distill_result_df = pd.DataFrame(columns=['round', 'distill_loss', 'hr', 'hdcg'])
    distill_result_df.to_csv("./result/obfus_distill_result.csv", mode='w', index=False)
...
def log_client_loss(client_id, local_epoch, loss):
    ...
    client_loss_df.to_csv("./result/obfus_client_loss.csv", mode='a', header=False, index=False)
...
def log_distill_result(rnd, distill_loss, hr, hdcg):
    ...
    distill_result_df.to_csv("./result/obfus_distill_result.csv", mode='a', header=False, index=False)
