...
class Server:
    def __init__(self, client_list, user_num, item_num, latent_dim, rounds):
        ...
        self.optimizers = [torch.optim.Adam(client.ncf.parameters(), lr=5e-4) for client in self.clients]
    ...
    def iterate(self, rnd=0):  # rnd -> round
        ...
        for client_id, client in bar:
            ...
            models_dict.append(client.ncf.state_dict())
    ...
    def run(self):
        for rnd in range(self.rounds):  # rnd -> round
            ...
            _ = [client.ncf.load_server_weights(self.server_model) for client in self.clients]
            self.iterate(rnd)
